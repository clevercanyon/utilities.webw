...
import { $chalk, $cmd, $fs } from '../../../../node_modules/@clevercanyon/utilities.node/dist/index.js';
import { $env, $version } from '../../../../node_modules/@clevercanyon/utilities/dist/index.js';
...
export default async () => {
        ...
        NODE_OPTIONS: $cmd.quote(
            $cmd
                .escAll([
                    ...
                    ...['--require', path.resolve($fs.imuDirname(import.meta.url), './resources/node.cjs')],
                ])
                .join(' '),
        ),
    };
    const cloudflareEnvVars = { CLOUDFLARE_API_TOKEN: $env.get('USER_CLOUDFLARE_TOKEN', { type: 'string' }) };
    ...
    return {
        ...
        'envs': async () => {
            return {
                env: { ...nodeEnvVars },
                cmds: [[path.resolve(u.dfBinDir, './envs.mjs'), '{{@}}']],
            };
        },
        'install': async () => {
            return {
                env: { ...nodeEnvVars },
                cmds: [[path.resolve(u.dfBinDir, './install.mjs'), '{{@}}']],
            };
        },
        'update': async () => {
            return {
                env: { ...nodeEnvVars },
                cmds: [[path.resolve(u.dfBinDir, './update.mjs'), '{{@}}']],
            };
        },
         ...
         */
        'wrangler': async ({ args }) => {
            const settings = await getWranglerSettings();
            return {
                env: { ...nodeEnvVars, ...cloudflareEnvVars },
                opts: { ...('pages' === args._?.[0] ? { cwd: u.distDir } : {}) },
                cmds: [
                    ...
                    ...('pages' === args._?.[0]
                        ? [
                              [
                                  ...
                                  ...('deploy' === args._?.[1] ? (args._?.[2] ? [] : [u.distDir]) : []),
